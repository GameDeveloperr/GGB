...
public class TutorialOrb : MonoBehaviour {
    ...
    {//플레이어 기준으로 적 판단
        ...
        Collider2D[] colls = Physics2D.OverlapCircleAll(pos, AttackDist);

        for (int i = 0; i != colls.Length; i++)
        {
            if (colls[i].CompareTag("TUTORIALENEMY") && Target == null)
            {
                Target = colls[i].gameObject.transform;
                //Debug.Log("OnTarget");
            }
            else if (colls[i].CompareTag("TUTORIALENEMY") && Target != null)
            {
